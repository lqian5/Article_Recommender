...
def load_glove(filename):
    ...
    for line in contents:
        content =line.split(' ')
        word = content[0]
        features = np.array(content[1:], dtype = 'float')
        words_database[word] = features
...
def words(text):
    ...
    """
    # regex compile to do what?
    # replace each occurence of pattern by ' ' and return the string
...
def load_articles(articles_dirname, gloves):
    ...
    """
    filelists = filelist(articles_dirname)
    pattern = '*.txt'
    table =[]
    for file in fnmatch.filter(filelists, pattern):
        # filename strip of root direction
        filename = re.sub(articles_dirname+'/','', file)

        contents = get_text(file)
        # find position of the first appearance of new line
        index = re.search('\n', contents).start()
        ...
        wordvec_centroid = doc2vec(text, gloves)

        table.append([filename, title, text, wordvec_centroid])
...
def doc2vec(text, gloves):
    ...
    for word in wordlist:
        if word in gloves:
            sumofvector += gloves[word]
            counter += 1

    return sumofvector/counter
...
def distances(article, articles):
    ...
    for art in articles:
        if art[0] != article:
            distance = sum((art[3] - centroid)**2)
            distance = distance**0.5
            # filename and title are the second element in the distance tuple
            distance_list.append((distance, (art[0], art[1])))
...
def recommended(article, articles, n):
    ...
    """
    Dlist = distances(article, articles)
    return sorted(Dlist, key=lambda a:a[0])[:5]
